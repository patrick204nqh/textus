...
- **Operation** — how you act on things: RPC and CLI verbs (`get`, `put`, `fetch`, `publish`, …).
...
`.textus/audit.log` is an append-only NDJSON file written under a file lock by every successful `put`, `delete`, `accept`, and `publish`. `.textus/role` (one line containing a role name) is optional and participates in the role-resolution order (§5).
...
| `artifacts` | `derived` | `build` | `automation` | Computed outputs (catalogs, indexes, published context). Written via `textus publish`. |
...
| `publish [--prefix=K] [--dry-run]` | write | `build`-holder (typically `automation`) |
...
Each row reports one entry's verdict (`fresh`, `stale`, `never_fetched`, or `no_policy`) against its matched `fetch:` rule. `textus publish` consumes its own staleness signal and executes derived entries' projections under a `build`-holding role (`automation` by default); `--dry-run` prints the plan without executing.
...
Given a manifest entry `derived.catalogs.skills` whose `compute: { kind: projection }` clause selects fields from `working.projects` entries, `textus publish derived.catalogs.skills` materializes the derived entry on disk with frontmatter and body matching the projected shape, and updates `generated.at` to the publish timestamp.

**Fixture F — Mustache render:**
Given a derived entry with a `template` clause referencing a `.mustache` file and inputs drawn from other keys, `textus publish` produces a body whose contents match the expected rendered output byte-for-byte (after trailing-newline normalization).

**Fixture G — Copy publish:**
Given a manifest entry with `publish: { to: [<path>] }`, a successful `textus publish` for that entry leaves a plain file at `<path>` whose contents are byte-identical to the in-store artifact at `.textus/zones/<...>`, accompanied by a sentinel at `.textus/sentinels/<path>.textus-managed.json` recording `source`, `target`, `sha256`, and `mode: "copy"`. Re-running `publish` is idempotent.

**Fixture H — Audit log format:**
Every successful write verb (`put`, `delete`, `publish`, `accept`, `schema migrate`) appends exactly one line per affected key to the audit log, in the canonical format defined in §audit (timestamp, actor role, verb, key, etag-before, etag-after). No write produces zero or multiple lines per key.
