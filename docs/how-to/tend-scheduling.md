...
`drain` is the full two-phase maintenance pass (ADR 0087):
...
(Intake refresh **is** part of the `drain` sweep — a stale `action: refresh`
...
(ADR 0078). The host owns the timer; `drain` is the verb it calls.
...
`upkeep: { strategy: async }` rule. `drain` is the on-demand catch-all for full recomputation or
...
```bash
textus drain --zone feeds --as=automation
```

## Authority

`drain` self-elevates for Phase 1 (materializing derived entries is a pure,
...
`drain` MCP tool over the same connection it uses for the edit→accept loop,
