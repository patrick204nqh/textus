...
2. **No new key — the publish *unit* is derived from the entry's shape, which `index_filename` already declares.**
   ...
   The mode is taken from `index_filename`, **never inferred from the template string** (which can't distinguish a dir from an extensionless file) — so nothing is silently reinterpreted. This is derive-or-guard (ADR 0037/0039): the `{basename}`/`{ext}` vars (`nested.rb:36`) are meaningless for a directory target and their use in a directory-leaf template is a **validation error**; `doctor` additionally warns if a directory-leaf template ends in the `index_filename` (the likely-mistaken "copy into a dir named `SKILL.md/`" shape). Publish does not re-declare a shape the entry already owns.
...
- **Manifest-schema addition** (`publish_tree`), but **no `textus/N` wire change** — publish is repo-local materialization. Per the `adr` runbook step 4, `SPEC.md` gets the `publish_tree` key alongside `publish_each`; behavior (subtree copy, ignore, prune, per-file sentinel) is the *why* recorded here.
...
- **Overload `publish_each` to detect dir vs file** (e.g. trailing `/`, or absence of `{basename}`/`{ext}`). Rejected: implicit and fragile. A distinct `publish_tree` key states intent and validates cleanly.
