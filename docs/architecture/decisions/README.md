...
| [0079](./0079-unify-lifecycle-policy.md) | Unify staleness + retention into one `lifecycle: {ttl, on_expire}` policy (`on_expire ∈ refresh\|warn\|drop\|archive`); an action's *destructiveness* decides its execution site — non-destructive `refresh`/`warn` run lazily on `get`/`list`, destructive `drop`/`archive` run only on the `tend` sweep; collapse the upkeep surface 8→3 (remove `stale`/`retainable`/`fetch`/`fetch_all`/`retain`), `tend` becomes the destructive-only sweep. Breaking: manifest schema + verb surface + SPEC; needs a `migrate` rewrite. Generator/build staleness stays out (dependency-based, not age-based) | Proposed |
