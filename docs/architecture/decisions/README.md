...
| [0062](./0062-one-get-read-through.md) | One `get`: unify the public read verb on read-through (fetch-on-stale, degrades to a pure read); keep `Read::GetEntry` as an internal pure primitive; drop the `get_or_fetch` verb | Accepted (ships 0.44.0) |
