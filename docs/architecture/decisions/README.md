...
| [0070](./0070-content-addressed-build-artifacts.md) | Built artifacts are content-addressed — the builder stamps no `generated_at`; `IdempotentWrite` deleted (byte-equality suffices), publish `cp` becomes a content no-op, the `sentinel.drift` class collapses; enables git-ignoring sentinels (defers the move) | Accepted |
