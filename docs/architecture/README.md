<!-- Generated by textus from the live manifest (lanes, roles) + static layer layout.
     Do not edit by hand — edit .textus/templates/docs/architecture-index.erb and
     run `textus drain` to refresh. -->

# Textus architecture

> **Explanation** · for contributors · **read this first** for orientation before SPEC
> **SSoT for** the Ruby implementation layout (layers, container, ports, dispatch paths)

## Lanes

| Name | Kind | Writers | Purpose |
|---|---|---|---|
| `knowledge` | `canon` | `human` | the maintained source of truth about the repo |
| `scratchpad` | `workspace` | `agent` | the agent's own durable scratchpad (working memory) |
| `proposals` | `queue` | `human`, `agent` | changes awaiting a human accept |
| `artifacts` | `machine` | `automation` | machine-maintained computed outputs |
| `raw` | `raw` | `human`, `agent`, `automation` | ingested external source material (write-once) |
...
produce/render.rb         ERB template expansion for publish targets
```
...
| `Ports::Clock` | Supplies `Time.now` — instantiable class so tests can inject. |
...
3. If no workflow matches but the entry has publish targets: runs `publish_only`.
