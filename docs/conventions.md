...
.textus/
  ...
  zones/
    identity/     # identity, voice, slow-changing facts — accept-holders only
    working/      # active working memory — accept-holders write directly
    intake/       # automation-fed external inputs (fetch)
    review/       # AI proposals awaiting accept (propose)
    output/       # generated by build automation — never edit by hand
```
...
```sh
textus fetch intake.notion.roadmap --as=automation
textus fetch stale --zone=intake --as=automation   # everything past its TTL
```
