...
|---|---|---|---|
| This repo (dogfooding) | textus developers | tracked `.claude/settings.json` `hooks.SessionStart` → `bundle exec exe/textus boot --lean` (the working-tree binary, like `.mcp.json`) | **Yes** — this repo un-ignores `.claude/settings.json` and commits the dogfood hook so every clone is wired |
| Shipped plugin | end users (and any repo that enables it) | inline `hooks` in `.claude-plugin/plugin.json` (`textus boot --lean`, the installed binary) | **Yes** — the shareable delivery |

The two channels differ only in *which binary* runs: the tracked dogfood `.claude/settings.json` shells the **working tree** (`bundle exec exe/textus`, matching `.mcp.json`), while the plugin shells the **installed** `textus`. This repo commits both — settings.json for its own development clones, the plugin manifest for distribution. (Earlier drafts assumed `.claude/settings.json` was git-ignored and concluded the plugin was the *only* committable path; the repo instead chose to track settings.json, so the dogfood hook is committed too. The agent-invokable `boot` tool stays regardless — the hook is additive, not a replacement. A developer who both clones this repo *and* enables the plugin would fire the hook twice; harmless, and not a real configuration since the plugin targets other repos.)

**Plugin layout — keep it self-contained, out of the gem tree.** The plugin is a *separate concern* from the textus gem, so all of its data lives in the manifest: a single `.claude-plugin/plugin.json` with the hook declared **inline** (`hooks` accepts an inline object, not only a `hooks/hooks.json` file). This avoids a top-level `hooks/` directory polluting the gem repo root, and sidesteps the convention that `.claude-plugin/` is manifest-only (an inline `hooks` value *is* manifest content). The MCP-server registration may likewise move into the manifest (`mcpServers` inline) in a later step — see [ADR 0086](./0086-textus-owns-agent-integration-config.md).

**SessionStart schema (verified):** `SessionStart` is an array of matcher-groups; each declares a `matcher` (`startup`/`resume`/`clear`/`compact`) and a nested `hooks` array. We register `startup`, `clear`, and `compact` (the cases where context is fresh or was lost); `resume` is skipped (context is preserved). Plain stdout from the command is injected as `additionalContext` — no JSON wrapper needed. `plugin.json`'s `hooks` field accepts an inline object (used here) or a path to a hooks file; an inline value keeps the manifest self-contained.
