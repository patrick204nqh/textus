...
- **A strategy-injection facade** (one `Read::Get` class taking `strategy: :pure | :read_through`). Rejected *as originally drafted*: it hides a collaborator behind a kwarg and adds a `strategy: :pure` footgun an internal caller could forget to pass (re-opening the reentrancy hazard a default would mask). **Reversed by the amendment above** — the shipped design is a single class with a `fetch:` mode flag, the very shape this bullet rejected. The footgun is closed by making the *method* default the safe one (`fetch: false` → pure; a forgetful direct caller lands on the safe read, not the dangerous one), keeping read-through as the *contract* default injected only on public surfaces, and building the orchestrator lazily so a pure call never constructs it (spec-guarded). With the safe default and lazy orchestrator, the mode-parameter shape is strictly cleaner than the two-class composition, not a footgun.
