  ...
  class Store
    ...
    class Sentinel
      ...
      def self.write!(target:, source:, store_root:)
        FileUtils.mkdir_p(File.dirname(sentinel_path(target, store_root)))
        repo_root = File.dirname(store_root)
        File.write(sentinel_path(target, store_root), JSON.generate(
      ...
      def self.sentinel_path(target, store_root)
        repo_root = File.dirname(store_root)
        rel = rel_or_abs(target, repo_root)
        rel = File.basename(target) if File.absolute_path?(rel)
      ...
      def self.rel_or_abs(path, repo_root)
        ...
        return path unless path.start_with?(base + File::SEPARATOR)
