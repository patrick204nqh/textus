module Textus
  module Mustache
    MAX_DEPTH = 8
    TAG = /\{\{(?<sigil>[#^\/!&]?)\s*(?<name>[\w.-]+)\s*\}\}/

    def self.render(template, context, strict: false, depth: 0)
    ...
    def self.parse_section(template, open_match, name)
      ...
      close_re = /\{\{\/\s*#{Regexp.escape(name)}\s*\}\}/
      ...
      while (m = template.match(both, cursor))
        if m[0].start_with?("{{/")
          depth -= 1
          if depth.zero?
            return [template[open_match.end(0)...m.begin(0)], m.end(0)]
          end
        else
          depth += 1
        end
