module Textus
  module Dispatch
    module Runtime
      # Drains the job queue: lease a job, look up its action in Dispatcher, run
      # it through Gate, then ack on success or fail on a raise. `drain` runs
      # until the queue is empty and returns a summary. Delivery is at-least-once.
      class Worker
