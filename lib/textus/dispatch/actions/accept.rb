    ...
    module Actions
      class Accept < WriteVerb
        ...
        def call(container:, call:)
          env = Textus::Dispatch::Actions::Get.new(key: @pending_key).call(container: container, call: call)
          ...
          case action
          when "put"
            Textus::Dispatch::Actions::Put.new(
              key: target,
              meta: env.meta["_meta"] || {},
              body: env.body,
            ).call(container: container, call: call)
          when "delete"
            Textus::Dispatch::Actions::KeyDelete.new(key: target).call(container: container, call: call)
          else
            raise Textus::ProposalError.new("unknown action: #{action}")
          end

          Textus::Dispatch::Actions::KeyDelete.new(key: @pending_key).call(container: container, call: call)
