    ...
    module Actions
      class KeyMvPrefix < Base
        ...
        def call(container:, call:)
          ...
          leaves = Textus::Dispatch::Actions::List.new(prefix: @from_prefix).call(container: container)
          ...
          plan = Textus::Maintenance::Plan.new(steps: steps, warnings: warnings)
          return plan if @dry_run

          steps.each do |step|
            Textus::Dispatch::Actions::KeyMv.new(old_key: step["from"], new_key: step["to"]).call(container: container, call: call)
          end
          plan
        end
      end
