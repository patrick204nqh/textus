...
RSpec.describe Textus::Application::Reads::GetOrRefresh do
  ...
  it "delegates to Get and skips orchestrator when verdict is fresh" do
    Dir.mktmpdir do |root|
      ...
      pure_get = Textus::Application::Reads::Get.new(ctx: ctx, caps: Textus::Application.caps_from_store(store)[0])
      orch = Class.new { def execute(*) = raise("must not call") }.new
      use_case = described_class.new(caps: Textus::Application.caps_from_store(store)[0], get: pure_get, orchestrator: orch)
