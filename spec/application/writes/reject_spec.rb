...
RSpec.describe Textus::Application::Writes::Reject do
  ...
  it "deletes the proposal and fires :proposal_rejected" do
    Dir.mktmpdir do |root|
      ...
      res = described_class.new(ctx: ctx, bus: store.bus).call("review.draft")
  ...
  it "rejects non-human callers" do
    Dir.mktmpdir do |root|
      ...
      expect { described_class.new(ctx: ctx, bus: store.bus).call("review.draft") }
  ...
  it "rejects entries that are not in a proposal zone" do
    Dir.mktmpdir do |root|
      store = build_store(File.join(root, ".textus"))
      ctx = Textus::Application::Context.new(store: store, role: "human")
      expect { described_class.new(ctx: ctx, bus: store.bus).call("identity.target") }
