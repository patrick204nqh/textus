...
RSpec.describe Textus::Application::Restructure::RuleLint do
  ...
  it "returns ok: true and zero diff lines when candidate is identical" do
    result = described_class.new(ctx: ctx, caps: Textus::Application.caps_from_store(store)[1]).call(
  ...
  it "reports an added rule" do
    ...
    result = described_class.new(ctx: ctx, caps: Textus::Application.caps_from_store(store)[1]).call(candidate_yaml: candidate)
  ...
  it "errors on an invalid candidate" do
    expect do
      described_class.new(ctx: ctx, caps: Textus::Application.caps_from_store(store)[1]).call(candidate_yaml: "this is not yaml: : :")
