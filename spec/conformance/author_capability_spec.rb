...
RSpec.describe "author capability (ADR 0033)" do
  ...
  it "lets an author-holder write a canon zone" do
    store.with_role("human").put(key: "knowledge.notes.x", meta: { "name" => "x" }, body: "hi\n")
    expect(store.with_role("human").get(key: "knowledge.notes.x").body).to eq("hi\n")
  end
  ...
  it "fails accept/reject with author_held when the role lacks `author`" do
    store.with_role("agent").put(key: "proposals.notes.p1",
                                 meta: { "name" => "p1",
                                         "proposal" => { "target_key" => "knowledge.notes.p1", "action" => "put" } },
                                 body: "please add\n")
    expect { store.with_role("agent").accept(pending_key: "proposals.notes.p1") }.to fail_guard_with("author_held")
  end
end
