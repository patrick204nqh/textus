...
RSpec.describe "workspace lane-kind + keep capability (ADR 0033)" do
  ...
  it "lets a keep-holder write its workspace directly — no accept needed (closes the agent-memory gap)" do
    store.with_role("agent").entry(:put, key: "notebook.notes.session1", meta: { "name" => "session1" }, body: "learned X\n")
    expect(store.with_role("agent").entry(:get, key: "notebook.notes.session1").body).to eq("learned X\n")
  end

  it "refuses a role that lacks the keep capability" do
    expect do
      store.with_role("human").entry(:put, key: "notebook.notes.x", meta: { "name" => "x" }, body: "")
