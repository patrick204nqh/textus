...
RSpec.describe "source/retention manifest load (ADR 0093)" do
  ...
  def load(manifest)
    store_from_manifest(root, lanes: %w[feeds knowledge], manifest: manifest).manifest
  end
    ...
    m = load(<<~YAML)
      ...
      entries:
        - key: feeds.doc
          ...
          lane: feeds
          source: { from: fetch, handler: h, ttl: 1h }
      rules:
  ...
  it "rejects an unknown key inside source:" do
    expect { load(<<~YAML) }.to raise_error(Textus::BadManifest, /unknown key 'bogus'/)
      ...
      lanes: [{ name: feeds, kind: machine }]
      entries:
        - { key: feeds.cat, kind: produced, path: data/feeds/cat.md, lane: feeds, source: { from: template, template: c, bogus: 1 } }
    YAML
  end
end
