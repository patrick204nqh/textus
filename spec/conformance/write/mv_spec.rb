...
RSpec.describe "textus mv" do
  ...
  def put_md(key, body: "hi")
    ...
    store.with_role("human").entry(:put, key, meta: { "name" => basename }, body: body)
  end

  it "moves an entry within the same zone, preserving uid" do
    env = put_md("knowledge.notes.alpha")
    uid = env.uid
    res = store.with_role("human").entry(:key_mv, "knowledge.notes.alpha", "knowledge.notes.beta")
    ...
    expect(store.with_role(Textus::Value::Role::DEFAULT).entry(:get, "knowledge.notes.beta").uid).to eq(uid)
  end

  it "writes an audit row with verb=mv and top-level structural fields" do
    put_md("knowledge.notes.alpha")
    store.with_role("human").entry(:key_mv, "knowledge.notes.alpha", "knowledge.notes.beta")
  ...
  it "refuses cross-zone moves" do
    put_md("knowledge.notes.alpha")
    expect do
      store.with_role("human").entry(:key_mv, "knowledge.notes.alpha", "identity.notes.alpha")
  ...
  it "refuses when the new key fails grammar" do
    put_md("knowledge.notes.alpha")
    expect do
      store.with_role("human").entry(:key_mv, "knowledge.notes.alpha", "knowledge.notes.Bad_Name")
  ...
  it "mints a uid if the source had none, so the audit row carries it" do
    ...
    File.write(src, "---\nname: alpha\n---\nbody\n")
    expect(store.with_role(Textus::Value::Role::DEFAULT).entry(:get, "knowledge.notes.alpha").uid).to be_nil

    res = store.with_role("human").entry(:key_mv, "knowledge.notes.alpha", "knowledge.notes.beta")
    expect(res["uid"]).to match(/\A[a-f0-9]{12,}\z/)
    expect(store.with_role(Textus::Value::Role::DEFAULT).entry(:get, "knowledge.notes.beta").uid).to eq(res["uid"])
