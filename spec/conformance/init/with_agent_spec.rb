  ...
  describe "agent-memory promise (ADR 0033)" do
    it "lets the agent write its scratchpad without a human accept" do
      Dir.mktmpdir do |dir|
        ...
        store.with_role("agent").put(key: "scratchpad.notes.s1", meta: { "name" => "s1" }, body: "remembered\n")
        expect(store.with_role("agent").get(key: "scratchpad.notes.s1").body).to eq("remembered\n")
      end
    end
  end
