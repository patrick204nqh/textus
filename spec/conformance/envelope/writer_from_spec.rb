...
RSpec.describe "Textus::Store::Envelope::Writer.from" do
  ...
  it "builds a Writer wired from the container that round-trips a put" do
    ...
    expect(writer).to be_a(Textus::Store::Envelope::Writer)
  ...
  it "behaves identically to a hand-wired .new" do
    ...
    mentry = store.manifest.resolver.resolve("knowledge.foo").entry
    payload = Textus::Store::Envelope::Writer::Payload.new(meta: {}, body: "x", content: nil)

    from_writer = Textus::Store::Envelope::Writer.from(container: container, call: call)
