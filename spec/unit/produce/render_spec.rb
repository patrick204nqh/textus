...
RSpec.describe Textus::Produce::Render do
  ...
  it "renders entry data through the target template" do
    target = Textus::Manifest::Policy::PublishTarget.new("to" => "OUT.md", "template" => "t")
  ...
  it "raises on a verbatim target (caller copies, not renders)" do
    target = Textus::Manifest::Policy::PublishTarget.new("to" => "out.json")
  ...
  it "injects boot when asked" do
    ...
    target = Textus::Manifest::Policy::PublishTarget.new("to" => "OUT.md", "template" => "t", "inject_boot" => true)
