...
RSpec.describe Textus::Produce::Render::Context do
  ...
  describe ".for" do
    ...
    it "exposes locals as methods on the returned binding" do
      result = eval("title", ctx.to_erb_binding, __FILE__, __LINE__)
    ...
    it "exposes textus_link as a method when resolver is present" do
      ...
      result = eval("textus_link('some.key')", ctx.to_erb_binding, __FILE__, __LINE__)
    ...
    it "records an edge in the edge_store on successful textus_link resolution" do
      ...
      eval("textus_link('some.key')", ctx.to_erb_binding, __FILE__, __LINE__)
    ...
    it "does not record an edge when resolver raises UnknownKeyError" do
      ...
      eval("textus_link('missing.key')", ctx.to_erb_binding, __FILE__, __LINE__)
    ...
    it "does not expose textus_link when resolver is nil" do
      ...
      expect { eval("textus_link('foo')", ctx_no_resolver.to_erb_binding, __FILE__, __LINE__) }.to raise_error(NameError)
    end
  end
end
