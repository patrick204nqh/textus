...
RSpec.describe Textus::Surfaces::MCP::Session do
  let(:contract_etag) { Digest::SHA256.hexdigest("contract-bytes") }

  it "carries role, cursor, propose_lane, contract_etag" do
    s = described_class.new(
      role: "agent", cursor: 42, propose_lane: "review", contract_etag: contract_etag,
    )
    expect(s.role).to eq("agent")
    expect(s.cursor).to eq(42)
    expect(s.propose_lane).to eq("review")
    expect(s.contract_etag).to eq(contract_etag)
  end

  it "advances cursor, returning a new session (immutable)" do
    s = described_class.new(role: "agent", cursor: 1, propose_lane: nil, contract_etag: "x")
