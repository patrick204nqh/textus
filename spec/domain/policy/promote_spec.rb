...
RSpec.describe Textus::Domain::Policy::Promote do
  it "accepts known requirements" do
    p = described_class.new(requires: %w[schema_valid accept_signed])
    expect(p.requires).to contain_exactly(:schema_valid, :accept_signed)
  end
  ...
  it "exposes a #demands?(:requirement) predicate" do
    ...
    expect(p.demands?(:accept_signed)).to be(false)
  end

  it "accepts the :accept_signed predicate" do
    p = described_class.new(requires: [:accept_signed])
    expect(p.demands?(:accept_signed)).to be true
  end
