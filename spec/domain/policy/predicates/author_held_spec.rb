...
RSpec.describe Textus::Domain::Policy::Predicates::AuthorHeld do
  ...
  it "fails and sets reason for an actor without 'author' (no bespoke #error — folds into GuardFailed)" do
    ...
    expect(pred.reason).to match(/lacks the 'author' capability/)
    expect(pred).not_to respond_to(:error)
  end

  it "fails with a disabled message when no role holds 'author'" do
    allow(policy).to receive(:roles_with_capability).with("author").and_return([])
    pred = described_class.new
    expect(pred.call(eval_for("human"))).to be(false)
    expect(pred.reason).to match(/no role holds the 'author' capability/)
  end
end
