require "spec_helper"

RSpec.describe "Read::SchemaEnvelope MCP contract (ADR 0039)" do
  let(:store) { Textus::Store.discover(File.expand_path("../../examples/project", __dir__)) }

  it "is exposed on MCP under the name 'schema', keyed by entry key" do
    c = Textus::Read::SchemaEnvelope.contract
    expect(c.verb).to eq(:schema)
    expect(c.mcp?).to be(true)
    expect(c.args.map(&:name)).to eq([:key])
  end

  it "the MCP catalog dispatches schema by key" do
    session = store.session(role: "human")
    out = Textus::MCP::Catalog.call("schema", session: session, store: store,
                                              args: { "key" => "knowledge.project" })
    expect(out["key"]).to eq("knowledge.project")
    expect(out).to have_key("schema")
  end
end
