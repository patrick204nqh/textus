...
RSpec.describe "Manifest format: field validation" do
  ...
  it "rejects .md path with format: json" do
    write_manifest("  - { key: working.x, path: working/x.md, zone: working, format: json }")
  ...
  it "rejects text format with a schema" do
    write_manifest("  - { key: working.x, path: working/x.txt, zone: working, format: text, schema: foo }")
    expect { Textus::Manifest.load(root) }
  ...
  it "globs both .yaml and .yml for nested yaml entries" do
    write_manifest(<<~YAML)
      - { key: working.cfg, path: working/cfg, zone: working, format: yaml, nested: true }
    YAML
    ...
    keys = manifest.enumerate.map { |r| r[:key] }.sort
