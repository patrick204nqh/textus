...
RSpec.describe Textus::Ports::BuildLock do
  ...
  it "writes pid/start/host diagnostic content while held" do
    ...
    described_class.with(root: root) do
      captured = File.read(Textus::StoreGeometry.new(root).lock_path("build"))
    end
  ...
  it "auto-releases when the holding process dies (kernel releases FD)" do
    ...
    pid = fork do
      ...
      file = File.open(Textus::StoreGeometry.new(root).lock_path("build"), File::RDWR | File::CREAT, 0o644)
