...
RSpec.describe Textus::Ports::AuditLog do
  ...
  describe "#verify_integrity" do
    ...
    it "detects a seq gap (missing row)" do
      FileUtils.mkdir_p(Textus::StoreGeometry.new(root).audit_dir_path)
      path = Textus::StoreGeometry.new(root).audit_log_path
  ...
  describe "textus/4 role canonicalization" do
    ...
    it "tolerates pre-0.11.0 legacy role values (ai/script/build) verbatim" do
      ...
      FileUtils.mkdir_p(Textus::StoreGeometry.new(root).audit_dir_path)
      File.write(
        Textus::StoreGeometry.new(root).audit_log_path,
