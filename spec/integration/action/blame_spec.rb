...
RSpec.describe Textus::Action::Blame do
  ...
  def write_audit(store, row)
    FileUtils.mkdir_p(Textus::StoreGeometry.new(store.root).audit_dir_path)
    File.open(Textus::StoreGeometry.new(store.root).audit_log_path, "a") { |f| f.puts JSON.generate(row) }
  end
