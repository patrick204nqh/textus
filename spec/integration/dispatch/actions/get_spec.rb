...
RSpec.describe Textus::Dispatch::Actions::Get do
  ...
  it "returns the on-disk envelope, observing staleness without ingesting" do
    ...
    env = described_class.new(key: "knowledge.doc").call(container: store.container, call: ctx)
  ...
  it "returns nil when the key has no envelope" do
    ...
    ctx = test_ctx(role: "automation")
    use_case = ->(key) { described_class.new(key: key).call(container: store.container, call: ctx) }
  ...
  it "annotates as fresh when no source.ttl applies (non-intake leaf)" do
    ...
    env = described_class.new(key: "feeds.doc").call(container: store.container, call: call)
