...
RSpec.describe Textus::Port::Publisher do
  ...
  let(:sentinel) { File.join(store_root, ".state", "tracking", "sentinels", "dst.md.textus-managed.json") }
  ...
  it "writes the sentinel under <store_root>/.state/tracking/sentinels/ with repo-relative source/target fields" do
    publisher.publish(source: src, target: dst, store_root: store_root)
  ...
  it "mirrors nested target paths in the sentinel tree" do
    ...
    expected_sentinel = File.join(store_root, ".state", "tracking", "sentinels", ".claude-plugin", "marketplace.json.textus-managed.json")
