...
RSpec.describe Textus::Manifest::Rules do
  it "parses a guard map of transition => predicate specs" do
    rules = described_class.parse([
                                    { "match" => "working.**",
                                      "guard" => { "accept" => ["author_held", "schema_valid", { "fresh_within" => "1h" }] } },
                                  ])
    set = rules.for("working.notes.x")
    expect(set.guard).to eq({ "accept" => ["author_held", "schema_valid", { "fresh_within" => "1h" }] })
  end
  ...
  it "rejects a non-Hash guard:" do
    expect { described_class.parse([{ "match" => "working.**", "guard" => ["author_held"] }]) }
