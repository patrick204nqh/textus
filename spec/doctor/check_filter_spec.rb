...
RSpec.describe "Doctor checks filter" do
  ...
  it "Doctor.run accepts a checks: filter and runs only the named check" do
    ...
    res = Textus::Doctor.run_via(container: store.container, role: Textus::Role::DEFAULT, checks: ["schemas"])
  ...
  it "Doctor.run raises UsageError on unknown check name" do
    store = Textus::Store.discover(tmp)
    expect { Textus::Doctor.run_via(container: store.container, role: Textus::Role::DEFAULT, checks: ["bogus"]) }.to raise_error(Textus::UsageError)
  end
end
