...
RSpec.describe Textus::MCP::Session do
  let(:manifest_etag) { Digest::SHA256.hexdigest("manifest-bytes") }

  it "carries role, cursor, propose_zone, manifest_etag" do
    s = described_class.new(
      role: "agent", cursor: 42, propose_zone: "review", manifest_etag: manifest_etag,
    )
    expect(s.role).to eq("agent")
    expect(s.cursor).to eq(42)
    expect(s.propose_zone).to eq("review")
    expect(s.manifest_etag).to eq(manifest_etag)
  end

  it "advances cursor, returning a new session (immutable)" do
    s = described_class.new(role: "agent", cursor: 1, propose_zone: nil, manifest_etag: "x")
  ...
  it "raises ContractDrift if checked etag differs" do
    s = described_class.new(role: "agent", cursor: 0, propose_zone: nil, manifest_etag: "abc")
